...
export default {
  ...
  methods: {
    ...
    fieldError(selector) {
      return Array.from(document.querySelector(selector).classList).includes(
        "form-error"
      );
    },
    saveShow() {
      ...
      const showRef = firebase
        .database()
        .ref("users/" + firebase.auth().currentUser.uid + "/showsToAdd");
      const archiveRef = firebase
        .database()
        .ref("users/" + firebase.auth().currentUser.uid + "/formSubmissions");
          ...
          try {
            ...
            if (location.components.city) {
              this.showCells[5].content = `${location.components.city}, ${
                location.components.state_code
              }`;
            } else if (location.components.town) {
              this.showCells[5].content = `${location.components.town}, ${
                location.components.state_code
              }`;
            } else {
              this.showCells[5].content = `${location.components.county}, ${
                location.components.state_code
              }`;
            }
            ...
            if (this.showCells[6].content !== "") {
              this.addressDetails = `No location found for "${
                this.showCells[6].content
              }"`;
            } else {
